...
export default function Page() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-[color:var(--text)]">
      ...
      <div className="relative mx-auto max-w-[1600px] px-4 sm:px-6 py-4 sm:py-6 h-screen flex flex-col">

        {/* HeaderBar: Fixed height (140px min), no overlap */}
        <div className="shrink-0" style={{ minHeight: '120px' }}>
          <HeaderBar
        ...
        <div className="flex-1 min-h-0 pt-3 sm:pt-4">
          <div className="grid grid-cols-12 gap-4 sm:gap-6 h-full">
            ...
            <div className="col-span-12 lg:col-span-8 card card-accent relative flex flex-col p-1 overflow-hidden group">
              ...
              <div className="absolute top-3 left-3 sm:top-5 sm:left-5 z-[400] flex items-center gap-2 sm:gap-3">
                <div className="bg-black/60 backdrop-blur px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-white/10 flex items-center gap-1.5 sm:gap-2">
                  <Globe className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[color:var(--cyan)]" />
                  <span className="text-[10px] sm:text-xs font-bold tracking-widest text-white/90">LIVE MAP</span>
                </div>

                {/* Optional: lightweight mode chip (no time) */}
                <div className="hidden sm:block bg-black/40 backdrop-blur px-2.5 py-1 rounded-full border border-white/10">
            ...
            {/* Right Column: Presenter + Feed stacked */}
            <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 sm:gap-6 overflow-hidden">

              {/* Presenter Panel - now part of layout flow, not absolute overlay */}
              <SystemPresenter
              ...
              <div className="card card-accent flex-1 flex flex-col min-h-0">
                <div className="p-3 sm:p-4 border-b border-white/5 flex items-center justify-between">
                  <div className="text-[10px] sm:text-xs tracking-[0.2em] font-bold text-[color:var(--muted)] flex items-center gap-2">
              ...
              <div className="card p-3 sm:p-4 shrink-0">
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <div className="text-[9px] sm:text-[10px] tracking-widest text-[color:var(--muted)] font-bold">SYSTEM INTEGRITY</div>
                  <div className="w-1.5 h-1.5 rounded-full bg-[color:var(--lime)] shadow-[0_0_8px_var(--lime)]" />
                </div>
                <div className="space-y-1.5 sm:space-y-2">
                  <div className="flex justify-between text-[10px] sm:text-xs font-mono text-white/40">
        ...
        {/* Ticker Bottom */}
        <div className="card h-9 sm:h-10 mt-3 sm:mt-auto shrink-0 flex items-center overflow-hidden bg-black/60 backdrop-blur-md border-0 border-t border-white/5">
          <div className="px-3 sm:px-5 h-full flex items-center bg-[color:var(--rose)]/10 border-r border-white/5 text-[color:var(--rose)] text-[9px] sm:text-[10px] font-bold tracking-widest z-10 gap-1.5 sm:gap-2">
            <div className="w-1.5 h-1.5 bg-current rounded-full animate-pulse" />
            BREAKING
          </div>
          <div className="flex-1 relative h-full flex items-center overflow-hidden">
            <motion.div
              className="absolute whitespace-nowrap text-[11px] sm:text-xs font-mono font-medium text-[color:var(--text)] opacity-80"
              animate={{ x: ["100%", "-100%"] }}
